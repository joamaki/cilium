...
| enableRuntimeDeviceDetection | bool | `false` | Enables experimental support for the detection of new and removed datapath devices. On device change the eBPF datapath, IPsec and BandwidthManager functionality is reloaded to add and remove the device specific eBPF programs. If "devices" is set then those are taken as the starting point, but the set of used devices may be expanded at runtime. Device wildcards are currently ignored by this option. |
