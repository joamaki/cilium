...
func (g *GetNodesSuite) SetUpSuite(c *C) {
	var err error
	nm, err = manager.NewManager("", &backoff.ClusterSizeBackoff{}, &fakeConfig.Config{})
	c.Assert(err, IsNil)
}
...
func (g *GetNodesSuite) Test_getNodesHandle(c *C) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupArgs: func() args {
				nodeDiscovery := nodediscovery.NewNodeDiscovery(nm, &backoff.ClusterSizeBackoff{}, mtu.NewConfiguration(0, false, false, false, 0, nil), &cnitypes.NetConf{})
				return args{
					params: GetClusterNodesParams{
						ClientID: &zero,
					},
