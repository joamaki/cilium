...
func (ini *localNodeSynchronizer) parseNode(k8sNode *slim_corev1.Node) parsedNode {
	parsed := parsedNodeFromData(k8s.ParseNodeData(
		ini.Logger,
		k8sNode,
		source.Unspec,
		ini.ClusterInfo,
	))
	parsed.uid = k8sNode.GetUID()
	parsed.providerID = k8sNode.Spec.ProviderID
	return parsed
}
