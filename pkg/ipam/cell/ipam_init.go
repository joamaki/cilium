...
func (r *IPAMInitializer) ConfigureAndStartIPAM(ctx context.Context) {
	...
	if r.daemonConfig.IPv4Range != autoCIDR {
		...
		r.localNodeStore.Update(func(n *node.LocalNodeMutator) {
			n.SetAllocationCIDRs(false, allocCIDR)
		})
	}
	...
	if r.daemonConfig.IPv6Range != autoCIDR {
		...
		r.localNodeStore.Update(func(n *node.LocalNodeMutator) {
			n.SetAllocationCIDRs(true, allocCIDR)
		})
	}
...
func (r *IPAMInitializer) AutoComplete(ctx context.Context) error {
	...
	if r.daemonConfig.EnableIPv6 && !ln.AllocationCIDR(true).IsValid() {
		return fmt.Errorf("IPv6 allocation CIDR is not configured. Please specify --%s", option.IPv6Range)
	}

	if r.daemonConfig.EnableIPv4 && !ln.AllocationCIDR(false).IsValid() {
...
func (r *IPAMInitializer) setDefaultPrefix(device string, n *node.LocalNodeMutator) {
	localNode := n.Node()
	if r.daemonConfig.EnableIPv4 {
		...
		ipv4range := localNode.AllocationCIDR(false)
		ipv6range := localNode.AllocationCIDR(true)
	...
	if r.daemonConfig.EnableIPv6 {
		...
		if localNode.GetNodeIP(isIPv6) == nil {
			...
			n.SetAddress(node.AddressKindNode, addressing.NodeInternalIP, true, ip)
			localNode = n.Node()
		}
