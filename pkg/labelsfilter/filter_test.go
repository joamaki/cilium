...
func TestFilterLabels(t *testing.T) {
	wanted := labels.NewLabels(
		...
		labels.NewLabel("io.cilium.k8s.policy.serviceaccount", "luke", labels.LabelSourceContainer),
	)
	...
	filtered, _ = dlpcfg.filterLabels(allLabels)
	require.Equal(t, 7, filtered.Len())
	require.True(t, wanted.Equal(filtered))
...
func TestFilterLabelsDocExample(t *testing.T) {
	...
	wanted := labels.NewLabels(
		...
		labels.NewLabel("io.cilium.k8s.policy.serviceaccount", "luke", labels.LabelSourceK8s),
	)
	...
	require.Equal(t, 8, filtered.Len())
	...
	allLabels = allLabels.Add(labels.NewLabel("io.cilium.k8s.policy.serviceaccount", "luke", labels.LabelSourceK8s))
	filtered, _ = dlpcfg.filterLabels(allLabels)
	require.Equal(t, 10, filtered.Len())
