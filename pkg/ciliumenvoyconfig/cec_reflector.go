...
func registerCECReflector(
	...
	g job.Group,
	db *statedb.DB,
	tbl statedb.RWTable[*types.CEC],
) error {
	if lws.cec == nil || !ecfg.EnableExperimentalLB {
		return nil
	}
	transform := func(obj any) (*types.CEC, bool) {
		var (
	...
	err := k8s.RegisterReflector(
		g,
		db,
		k8s.ReflectorConfig[*types.CEC]{
			Name:          "cec",
			Table:         tbl,
			ListerWatcher: lws.cec,
			Transform:     transform,
			QueryAll: func(txn statedb.ReadTxn, tbl statedb.Table[*types.CEC]) iter.Seq2[*types.CEC, statedb.Revision] {
				return statedb.Filter(
					tbl.All(txn),
					func(cec *types.CEC) bool { return cec.Name.Namespace != "" },
				)
			},
			CRDSync: crdSync,
		},
