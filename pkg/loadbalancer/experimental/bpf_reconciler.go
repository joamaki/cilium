...
func registerBPFReconciler(p reconciler.Params, ops *bpfOps, w *Writer) error {
	...
	_, err := reconciler.Register(
		...
		// TODO: Using smaller values for testing. Revisit this.
...
type bpfOps struct {
	...
	// TODO: Clearly separate these into its own thing and test it well.
...
func newBPFOps(lc cell.Lifecycle, log *slog.Logger, lbmaps lbmaps) *bpfOps {
	ops := &bpfOps{
...
func (ops *bpfOps) start(_ cell.HookContext) error {
	// TODO: Map migrations? The migrations should happen before the loader starts,
	// so likely we want to keep that code in LBMap and have LBMap provide dependencies
	// towards loader. This code here should also be written against a suitable LBMap
	// interface (the current one does not work as it is too high-level).
...
func (ops *bpfOps) deleteFrontend(fe *Frontend) error {
	...
	if useMaglev(fe) {
		if err := lbmap.DeleteMaglevTable(fe.Address.IsIPv6(), uint16(feID)); err != nil {
			return fmt.Errorf("delete maglev %d: %w", feID, err)
		}
	}
...
func (ops *bpfOps) pruneRevNat() error {
	...
	cb := func(key lbmap.RevNatKey, value lbmap.RevNatValue) {
		revNatKey := key.(lbmap.RevNatKey).ToHost()
...
func (ops *bpfOps) updateFrontend(fe *Frontend) error {
	...
	var backendIDs []loadbalancer.BackendID
	...
	if useMaglev(fe) {
		lbmap.UpdateMaglevTable(fe.Address.IsIPv6(), uint16(feID), backendIDs)
	}
...
func useMaglev(fe *Frontend) bool {
	...
	// footprint.
	if fe.Type == loadbalancer.SVCTypeClusterIP && !option.Config.ExternalClusterIP {
		return false
	}
	// Wildcarded frontend is not exposed for external traffic.
	if fe.Type == loadbalancer.SVCTypeNodePort && fe.Address.AddrCluster.IsUnspecified() {
		return false
	}
