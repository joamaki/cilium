...
var (
	...
	nodePortAddr = loadbalancer.L3n4Addr{
		AddrCluster: types.MustParseAddrCluster("0.0.0.2"),
		L4Addr:      loadbalancer.L4Addr{},
		Scope:       0,
	}
...
func TestBPFOps(t *testing.T) {
	...
	for _, testCaseSet := range testCases {
		...
		for _, addr := range frontendAddrs {
			...
			// fresh IDs.
			ops := newBPFOps(lc, log, &realLBMaps{})
			for _, testCase := range testCaseSet {
				t.Run(testCase.name, func(t *testing.T) {
					...
					require.NoError(t, ops.Prune(nil, nil, nil), "Prune")
