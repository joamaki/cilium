...
func TestIntegrationK8s(t *testing.T) {
	...
	for _, ent := range dirs {
		...
		t.Run(ent.Name(), func(t *testing.T) {
			...
			//
			// FIXME: allow multiple stages in the style of test/controlplane.
			...
			if !bytes.Equal(actualTables, expectedTables) {
			...
			actual := dump(frontendAddrs[0], true)
