...
func TestHive(t *testing.T) {
	...
	hive := New(viper, flags,
		testCell,
		cell.OnStart(func(c Config) error {
			receivedConfig = true
			return nil
		}))
...
func TestOnStartOnStop(t *testing.T) {
	...
	onStart := func(s *SomeObject) error {
		if s.X != 10 {
			return errors.New("Not 10")
		}
		started = true
	...
	onStop := func(s *SomeObject) error {
		if s.X != 10 {
			return errors.New("Not 10")
		}
		stopped = true
	...
	if !started {
		t.Fatalf("OnStart hook was not called")
	}
	if !stopped {
		t.Fatalf("OnStop hook was not called")
	}
...
func TestRequire(t *testing.T) {
	...
	testCell := cell.Group(
		"test",
		cell.Provide(func() *SomeObject {
			constructed = true
			return &SomeObject{10}
		}),
		cell.Require[*SomeObject](),
	)
	...
	if !constructed {
		t.Fatal("expected *SomeObject to be constructed, but it was not")
	}
