...
func NewNameManager(config Config) *NameManager {
	...
	if config.UpdateSelectors == nil {
		config.UpdateSelectors = func(ctx context.Context, selectorsWithIPs map[api.FQDNSelector][]net.IP, selectorsWithoutIPs []api.FQDNSelector) (channels.DoneChan, []*identity.Identity, map[string]*identity.Identity, error) {
...
func (n *NameManager) UpdateGenerateDNS(ctx context.Context, lookupTime time.Time, updatedDNSIPs map[string]*DNSIPRecords) (done <-chan struct{}, usedIdentities []*identity.Identity, newlyAllocatedIdentities map[string]*identity.Identity, err error) {
	n.RWMutex.Lock()
