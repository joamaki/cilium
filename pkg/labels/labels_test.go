...
func testNewLabels(t *testing.T, n int) {
	...
	for _, l := range testLabels {
		...
		assert.True(t, lbls1.Has(l.Key()), "Has")
	}
...
func TestNewFrom(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.True(t, tt.want.Equal(tt.legacyLbls))
		})
	}
}
...
func TestSortMap(t *testing.T) {
	...
	lblsString += ";"
	sortedMap := legacyLbls.SortedList()
...
func TestLabelArraySorted(t *testing.T) {
	...
	lblsString += ";"
	str := ""
	for _, l := range ToLabelArray(legacyLbls) {
