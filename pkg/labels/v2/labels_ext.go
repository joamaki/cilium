...
func (lbls Labels) Contains(other Labels) bool {
	...
	if lbls.overflow == nil && other.overflow == nil && lbls.Len() == other.Len() {
		// Fast path, no overflow and same amount of labels. We can just compare the
		// handles directly.
		if lbls.handle == other.handle {
			return true
		}
	}
	if other.Len() > lbls.Len() {
		return false
	}
	...
	return func(yield func(string) bool) {
		for lbl := range l.All() {
			...
			if lbl.Source() == LabelSourceCIDR {
				if prefix, err := LabelToPrefix(lbl.Key()); err == nil {
					s = LabelSourceCIDR + SourceDelimiter + prefix.String()
				} else if cidr := lbl.CIDR(); cidr != nil {
					s = LabelSourceCIDR + SourceDelimiter + cidr.String()
				} else {
					s = lbl.String()
				}
			} else {
...
func (lbls Labels) AddWorldLabel(addr netip.Addr) Labels {
	...
	ls = append(ls, getWorldLabel(addr))
	return NewLabels(ls...)
}
